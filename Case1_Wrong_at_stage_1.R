...
Wrongfirstonly=function(kprime,kstar,theta0,sd,u,l,n) #all looking at a 2 stage trial 
{
  ...
  meansn=c(muZkprime0_1,muZkstar0_1,muZkstarkprime_1,muZkotherkprime_1)
  set.seed(1)
  ans=pmvnorm(lower=lbn,upper=ubn,mean = meansn,corr = Sigmapn)[[1]]
  return(ans)
}
...
Wrongfirstforward=function(theta0,sd,u,l,n) #Does it for whichever treatment is the best
{
  kstar=which.max(theta0)
  ...
  OPart3a=Wrongfirstonly(kprime = Nkstar[1],kstar,theta0,sd,u,l,n) #only need the fact we wrong not then correct
  
  OPart3b=Wrongfirstonly(kprime = Nkstar[2],kstar,theta0,sd,u,l,n) #only need the fact we wrong not then correct
  
  
  Wrongstage1=OPart3a+OPart3b
  
  return(list("Wrongstage1"=Wrongstage1))
}
...
Wrongfirstfocus <- foreach(x=1:dim(Mtheta0)[1]) %dopar% 
  {
    
    library(mvtnorm)
    Wrongfirstforward=Wrongfirstforward
    
    Wrongfirstonly=Wrongfirstonly
    ...
    tu=MAMStouse$u
    tsd=1
    ans=Wrongfirstforward(ttheta0,tsd,tu,tl,tn)
    Wrong1stgtheta0=ans$Wrongstage1
    
    return(list("giventheta0"=ttheta0,"Wrong1stgtheta0"=Wrong1stgtheta0))
  }


stopCluster(cl) 

save(Wrongfirstfocus,file = "Wrongfirstfocus.Rdata")
